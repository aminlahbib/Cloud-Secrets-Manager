...
**Level:** Beginner  
**Time:** 3 hours  
**Prerequisites:** Basic command line, GCP account
...
7. [Modules](#modules)
8. [Cloud Secrets Manager Infrastructure](#cloud-secrets-manager-infrastructure)
...
graph LR
    subgraph "Without Terraform"
        ...
        CON[GCP Console]
        INF1[Infrastructure]
        DEV1 -->|Click Click Click| CON
        CON -->|2 hours later| INF1
    end
    ...
    style CLI fill:#7B42BC,color:#fff
...
graph TB
    ...
    subgraph "Providers"
        GCP[Google<br/>Provider]
        AWS[AWS<br/>Provider]
        K8S[Kubernetes<br/>Provider]
    end
    ...
    GCP --> GCP_R[GKE, Cloud SQL<br/>Secret Manager]
    AWS --> AWS_R[EKS, RDS<br/>Secrets Manager]
    K8S --> K8S_R[Deployments<br/>Services]
...
## Cloud Secrets Manager Infrastructure
...
graph TB
    ...
    style M_GKE fill:#4285F4,color:#fff
    style M_SQL fill:#4285F4,color:#fff
```
...
graph LR
    ...
    subgraph "GCP Resources"
        GKE[GKE Cluster<br/>+ Node Pool]
        SQL[Cloud SQL<br/>PostgreSQL]
        AR[Artifact Registry]
        GSM[Secret Manager<br/>Secrets]
        PS[Pub/Sub<br/>Topics]
        SA[Service Accounts<br/>+ IAM]
    end
...
module "artifact_registry" {
  ...
  repositories = ["secret-service", "audit-service", "notification-service"]
}
```
