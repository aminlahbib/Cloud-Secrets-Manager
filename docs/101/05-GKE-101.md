...
**Level:** Intermediate  
**Time:** 3 hours  
**Prerequisites:** Kubernetes 101, Terraform 101
...
7. [Security](#security)
8. [Cloud Secrets Manager on GKE](#cloud-secrets-manager-on-gke)
...
graph TB
    subgraph "GKE Cluster"
        ...
        subgraph "Node Pool (You Pay For)"
            N1[Node 1<br/>e2-medium]
            N2[Node 2<br/>e2-medium]
            N3[Node 3<br/>e2-medium]
        end
    end
    ...
    style N3 fill:#34A853,color:#fff
```
...
| Feature | Description |
|---------|-------------|
| **Managed Control Plane** | Google manages master nodes |
| **Auto-scaling** | Scale nodes based on demand |
| **Auto-upgrades** | Automatic K8s version updates |
| **Cloud Integration** | Native GCP services |
...
sequenceDiagram
    ...
    TF->>GCP: Create Secret Manager
    TF->>GCP: Create Service Accounts
    GCP->>TF: Resources ready
    
    DEV->>HELM: helm install
    HELM->>GCP: Deploy to GKE
    GCP->>HELM: Pods running
```
...
module "gke" {
  ...
  cluster_name = "csm-cluster"
  region       = "us-central1"
  
  # VPC-native for private IPs
