...
## Artifact Registry & Kubernetes Sync (Updated Nov 21, 2025)

- **Registry:** `europe-west10-docker.pkg.dev/cloud-secrets-manager/docker-images`
- **Images pushed & tracked:** `secret-service:latest`, `audit-service:latest`
- **Kubernetes usage:** Every raw manifest and the Helm chart now pin to these registry tags, deploy into the `cloud-secrets-manager` namespace, and rely on the `artifact-registry-secret` imagePullSecret plus the `csm-*` resource/secret naming scheme (e.g., `csm-secret-service`, `csm-secrets-db`, `csm-app-config`).
- **Verification command:**  
  ```bash
  gcloud artifacts docker images list docker-images \
    --location=europe-west10 --project=cloud-secrets-manager
  ```
  (Run from a workstation with the gcloud CLI installed; this environment does not have gcloud available.)
- **Redeploy checklist:**  
  ```bash
...
- Deployable to cloud providers (Render / Railway / Azure)
...
## 3 JWT Authentication Flow - Code Templates
...
## 4 Microservice Split - Responsibilities
...
## 5 Deployment (Render / Railway / Azure)
