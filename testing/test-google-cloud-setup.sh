...
if [ -f "apps/backend/secret-service/src/main/resources/service-account.json" ]; then
    echo -e "${GREEN}✅ Service account file exists${NC}"
    
    # Check if it's valid JSON
    if python3 -m json.tool apps/backend/secret-service/src/main/resources/service-account.json > /dev/null 2>&1; then
        echo -e "${GREEN}✅ Service account file is valid JSON${NC}"
        
        # Extract project ID
        PROJECT_ID=$(cat apps/backend/secret-service/src/main/resources/service-account.json | grep -o '"project_id"[[:space:]]*:[[:space:]]*"[^"]*"' | cut -d'"' -f4)
    ...
    echo -e "${RED}❌ Service account file NOT found${NC}"
        echo "   Expected location: apps/backend/secret-service/src/main/resources/service-account.json"
fi
...
if grep -q "google:" apps/backend/secret-service/src/main/resources/application.yml && grep -q "cloud:" apps/backend/secret-service/src/main/resources/application.yml && grep -q "identity:" apps/backend/secret-service/src/main/resources/application.yml; then
    # Check for enabled flag (could be in format: enabled: ${GOOGLE_IDENTITY_ENABLED:true})
    ENABLED_LINE=$(grep -A 5 "google:" apps/backend/secret-service/src/main/resources/application.yml | grep -i "enabled" | head -1)
...
# Check project ID
PROJECT_ID_CONFIG=$(grep "project-id" apps/backend/secret-service/src/main/resources/application.yml | grep -o "cloud-secrets-manager[^\"]*" | head -1)
...
if curl -s http://localhost:8080/actuator/health > /dev/null 2>&1; then
    ...
    echo "   Or: cd apps/backend/secret-service && ./mvnw spring-boot:run"
fi
...
echo "   4. Create a test user or use your API to create one"
    echo "   5. Get ID token using: testing/postman/get-id-token.html"
